...
PATHS = {
    ...
    "elementaires": str(
        RAW_DIR / "AES" / "etablissements-scolaires-ecoles-elementaires.csv"
    ),
    ...
    "qos_habitations": str(
        RAW_DIR / "reseau" / "2025_QoS_Metropole_data_habitations.csv"
    ),
    # Réseau fibre
    "fibre": str(RAW_DIR / "reseau" / "carte_fibre_immeubles_2025_T4_20260130.csv"),
    # GeoJSON arrondissements (front)
    "arrondissements_geojson": str(
        BASE_DIR / "front" / "src" / "assets" / "arrondissements.geojson"
    ),
    # GeoJSON IRIS Paris (granularité fine, environ 992 zones)
    "iris_geojson": str(RAW_DIR / "map" / "iris.geojson"),
    # logements sociaux
    "logements_sociaux": str(
        RAW_DIR / "obligatoire" / "logements-sociaux-finances-a-paris.csv"
    ),
    # mobilité
    "velib": str(RAW_DIR / "mobilite" / "velib-emplacement-des-stations.csv"),
    "gares": str(RAW_DIR / "mobilite" / "emplacement-des-gares-idf.csv"),
    "arrets": str(RAW_DIR / "mobilite" / "arrets.csv"),
}
