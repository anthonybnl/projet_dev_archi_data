...
from pipeline.config import LAYERS, PATHS
...
def load_iris_gdf() -> gpd.GeoDataFrame:
    ...
    for feat in geojson["features"]:
        ...
        if props.get("dep") != "75":
            continue
        rows.append({"code_iris": props["code_iris"], "geometry": shape(feat["geometry"])})
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs("EPSG:2154")


def interpolate_qualite(qualite_iris: pd.DataFrame, iris_gdf: gpd.GeoDataFrame) -> pd.DataFrame:
    """
    ...
    for _ in range(3):
        ...
        for code in manquants:
            ...
            voisins_mask = iris_geo.geometry.touches(geom) | iris_geo.geometry.intersects(geom.boundary)
            ...
            valeurs_voisins = scores.loc[
                scores["code_iris"].isin(voisins_codes) & scores["score_qualite"].notna(),
                "score_qualite"
            ]
            if len(valeurs_voisins) > 0:
                scores.loc[scores["code_iris"] == code, "score_qualite"] = valeurs_voisins.mean().round(2)
...
def run(engine):
    silver = LAYERS["silver_schema"]
    gold   = LAYERS["gold_schema"]
    ...
    mobile_iris = mobile.groupby(["code_iris", "arrondissement"]).agg(
    ...
    print("[gold.score_reseau] Interpolation spatiale QoS pour les IRIS sans mesures...")
    iris_gdf = load_iris_gdf()

    if len(qualite) > 0:
        qualite_iris = qualite.groupby("code_iris").agg(
    ...
    result = result.merge(
        fibre[["code_iris", "taux_deploiement", "taux_pm_actif",
               "score_fibre", "meilleur_operateur_fibre"]],
    ...
    result["rang_reseau"] = result["score_final"].rank(ascending=False, method="min").astype(int)
    ...
    cols = [
        ...
        "couv_4g_max", "couv_5g_max",
        "taux_deploiement_fibre", "taux_pm_actif",
        "meilleur_operateur_mobile", "meilleur_operateur_fibre",
        "created_at",
    ]
    ...
    if inserted:
        ...
        print(result[["code_iris", "arrondissement", "rang_reseau", "score_final",
                       "meilleur_operateur_mobile", "meilleur_operateur_fibre"]].head(10).to_string(index=False))
    else:
